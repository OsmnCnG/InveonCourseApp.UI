...
export default function PaymentForm() {
  ...
  const { completePurchase } = useContext(CartContext);
  ...
  const handlePayment = async (e) =>{
    ...
    if(validateForm()){
      // await completePurchase(1,3); test
      await completePurchase(JSON.parse(localStorage.getItem("loginUser")).id);
      alertify.success("Ödeme başarılı!");
      navigate("/");
    }else{
      alertify.error("Ödeme işlemiyle ilgili bir sorun oluştu!");
    }
  };
  ...
  return (
    <div className='container mt-5 d-flex justify-content-center align-items-center'> 
        <div className='card shadow-lg p-4' style={{maxWidth: "500px", width: "100%"}}>
          <h2 className='text-center mb-3'>Ödeme bilgilerini giriniz</h2>
          <form onSubmit={handlePayment}>
                ...
                <button type='submit' className='btn btn-success w-100'>Şimdi Öde</button>
          </form>
